    ...
    public class MusConnection
    {
        ...
        public void processCommand(String data)
        {
            ...
            switch (header.ToLower())
            {
                ...
                case "reload_bans":
                    {
                        ...
                        PlusEnvironment.GetGame().GetClientManager().sendStaffMsg("La liste des utilisateurs bannis a été rechargée.");
                        break;
                    }
                #endregion
                #endregion
                ...
                case "confirmed":
                    {
                        ...
                        Room Room = Client.GetHabbo().CurrentRoom;
                        RoomUser User = Room.GetRoomUserManager().GetRoomUserByHabbo(Client.GetHabbo().Id);
                        Client.GetHabbo().Confirmed = 1;
                        User.OnChat(User.LastBubble, "* Devient un civil confirmé *", true);
                        if(Client.GetHabbo().Travaille == false && Client.GetHabbo().Prison == 0 && Client.GetHabbo().Hopital == 0 && Client.GetHabbo().footballTeam == null)
                ...
                case "register":
                    {
                        string nickname = Convert.ToString(Params[0]);

                        PlusEnvironment.GetGame().GetClientManager().sendStaffMsg(nickname + " vient de s'inscrire sur Waddow.");
                        break;
                    }
                #endregion
            ...
            log.Info("Commande MUS reçu: '" + header + "'");
        }
    }
}