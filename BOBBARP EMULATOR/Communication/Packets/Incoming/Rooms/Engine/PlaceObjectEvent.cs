    ...
    class PlaceObjectEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
                ...
                case InteractionType.MOODLIGHT:
                    {
                        ...
                        {
                            Session.SendNotification("Vous pouvez seulement avoir 1 lumière par appart.");
                            return;
                        }
                        break;
                    }
            ...
            if (!Item.IsWallItem)
            {
                ...
                if (Room.GetRoomItemHandler().CheckIfItemOnCase(7858, X, Y) && Item.GetBaseItem().SpriteId != 3289 && Item.GetBaseItem().SpriteId != 5273 || Room.GetRoomItemHandler().CheckIfItemOnCase(3289, X, Y) && Item.GetBaseItem().SpriteId != 5273)
                    return;

                if (Item.GetBaseItem().SpriteId != 5273 && Room.GetRoomItemHandler().CheckIfItemOnCase(5273, X, Y) && Item.GetBaseItem().SpriteId != 3289)
                    return;

                if (Item.GetBaseItem().SpriteId == 5273 && Room.GetRoomItemHandler().CheckIfItemOnCase(5273, X, Y))
                    return;

                if (Item.GetBaseItem().SpriteId == 3289)
                {
                    if (!Room.GetRoomItemHandler().CheckIfItemOnCase(7858, X, Y))
                    {
                        Session.SendWhisper("Vous devez planter la graine de cannabis dans un pot.");
                        return;
                    }

                    if (Room.GetRoomItemHandler().CheckIfItemOnCase(3289, X, Y))
                    {
                        Session.SendWhisper("Il y a déjà une graine de cannabis dans ce pot.");
                        return;
                    }

                    if (Math.Abs(User.Y - Y) > 1 || Math.Abs(User.X - X) > 1)
                    {
                        Session.SendWhisper("Vous devez être à coté du pot pour pouvoir planter une graine.");
                        return;
                    }

                    if (Session.GetHabbo().Eau < Convert.ToDecimal(0.20))
                    {
                        Session.SendWhisper("Il vous faut 0.20 litre d'eau pour pouvoir y planter une graine.");
                        return;
                    }
                ...
                if (Room.GetRoomItemHandler().SetFloorItem(Session, RoomItem, X, Y, Rotation, true, false, true))
                {
                    ...
                    if (RoomItem.GetBaseItem().SpriteId == 7858)
                    {
                        RoomItem.ExtraData = "1";
                        RoomItem.UpdateState();
                    }
                    else if (RoomItem.GetBaseItem().SpriteId == 3289)
                    {
                        Session.GetHabbo().Eau -= Convert.ToDecimal(0.20);
                        Session.GetHabbo().updateEau();

                        using (IQueryAdapter dbClient = PlusEnvironment.GetDatabaseManager().GetQueryReactor())
                        {
                            dbClient.SetQuery("INSERT INTO cannabis (item_id, user_id, last_arrosage) VALUES (@item_id, @user_id, NOW());");
                            dbClient.AddParameter("item_id", RoomItem.Id);
                            dbClient.AddParameter("user_id", Session.GetHabbo().Id);
                            dbClient.RunQuery();
                        }

                        User.OnChat(User.LastBubble, "* Plante une graine de cannabis dans le pot et l'arrose [-0.20L D'EAU] *", true);
                        RoomItem.ExtraData = "3";
                        RoomItem.UpdateState();
                    }
                    else if (RoomItem.GetBaseItem().SpriteId == 5273)
                    {
                        RoomItem.ExtraData = "1";
                        RoomItem.UpdateState();
                    }
