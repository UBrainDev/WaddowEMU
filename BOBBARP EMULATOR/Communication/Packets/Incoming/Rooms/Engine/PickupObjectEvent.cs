...
namespace Plus.Communication.Packets.Incoming.Rooms.Engine
{
    class PickupObjectEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            {
                Session.SendWhisper("Vous ne pouvez pas prendre la porte de votre appartement.");
                return;
            }

            if (Item.GetBaseItem().SpriteId == 7858 && Room.GetRoomItemHandler().CheckIfItemOnCase(3289, Item.GetX, Item.GetY))
            {
                Session.SendWhisper("Vous ne pouvez pas prendre ce pot dans votre inventaire car une plante y pousse.");
                return;
            }

            if (Item.GetBaseItem().SpriteId == 5273 && Room.GetRoomItemHandler().CheckIfItemOnCase(3289, Item.GetX, Item.GetY))
            {
                Session.SendWhisper("Vous ne pouvez pas prendre cet ampoule car elle permet le développement d'une plante.");
                return;
            }
