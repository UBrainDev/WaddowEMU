...
namespace Plus.Communication.Packets.Incoming.Rooms.Chat
{
    public class ShoutEvent : IPacketEvent
    {
        public void Parse(GameClient Session, ClientPacket Packet)
        {
            ...
            {
                string GoodReponse = PlusEnvironment.QuizzReponse;
                ...
                else
                {
                    foreach (RoomUser UserInRoom in Session.GetHabbo().CurrentRoom.GetRoomUserManager().GetUserList().ToList())
                    {
                        ...
                        byte[] encodeToUtf8 = Encoding.Default.GetBytes(GoodReponse);
                        GoodReponse = Encoding.UTF8.GetString(encodeToUtf8);
