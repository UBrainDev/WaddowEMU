...
namespace Plus.Communication.Packets.Incoming.Rooms.Chat
{
    public class WhisperEvent : IPacketEvent
    {
        public void Parse(GameClient Session, ClientPacket Packet)
        {
            ...
            PlusEnvironment.GetGame().GetChatManager().GetLogs().StoreChatlog(new Plus.HabboHotel.Rooms.Chat.Logs.ChatlogEntry(Session.GetHabbo().Id, Room.Id, "<Murmure à " + ToUser + ">: " + Message, UnixTimestamp.GetNow(), Session.GetHabbo(), Room));
            ...
            if (ToNotify.Count > 0)
            {
                foreach (RoomUser user in ToNotify)
                {
                    if (user != null && user.HabboId != User2.HabboId && user.HabboId != User.HabboId)
                    {
                        ...
                        {
                            user.GetClient().SendMessage(new WhisperComposer(User.VirtualId, "[Murmure à " + ToUser + "] " + Message, 0, User.LastBubble));
                        }
                    }
                }
            }
