...
namespace Plus.HabboHotel.Groups
{
    public class Group
    {
        ...
        public Group(int Id, string Name, string Description, string Badge, int RoomId, int Owner, int Time, int Type, int Colour1, int Colour2, int AdminOnlyDeco, int ChiffreAffaire, int PayedByEtat, int Usine)
        {
            this.Id = Id;
            this.Name = Name;
        ...
        public void updateChiffre()
        {
            using (IQueryAdapter dbClient = PlusEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.SetQuery("UPDATE `groups` SET chiffre = @chiffre WHERE id = @uid");
                dbClient.AddParameter("chiffre", this.ChiffreAffaire);
                dbClient.AddParameter("uid", this.Id);
                dbClient.RunQuery();
            }
        }

        public void retrograderRank(int Id)
        {
            if (!this.IsMember(Id))
                return;
