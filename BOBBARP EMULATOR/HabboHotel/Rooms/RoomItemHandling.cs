...
namespace Plus.HabboHotel.Rooms
{
    public class RoomItemHandling
    {
        ...
        public bool SetFloorItemByForce(GameClient Session, Item Item, int newX, int newY, int newRot, bool newItem, bool OnRoller, bool sendMessage, bool updateRoomUserStatuses = false)
        {
        ...
        public bool SetFloorItem(GameClient Session, Item Item, int newX, int newY, int newRot, bool newItem, bool OnRoller, bool sendMessage, bool updateRoomUserStatuses = false)
        {
            ...
            if (Session != null && Math.Abs(Session.GetHabbo().StackHeight) > Math.Pow(1, -9))
        ...
        public List<Item> RemoveItems(GameClient Session)
        {
            ...
            {
                if (Item == null || Item.GetBaseItem().SpriteId == 2693 || Item.GetBaseItem().SpriteId == 3289)
                    continue;
        ...
        public bool CheckIfItemOnCase(int spriteId, int X, int Y)
        {
            foreach (Item Item in GetFloor)
            {
                if (Item.GetBaseItem().SpriteId == spriteId && Item.GetX == X && Item.GetY == Y)
                    return true;
            }
        ...
        public Item getItemOnCase(int spriteId, int X, int Y)
        {
            foreach (Item Item in GetFloor)
            {
                if (Item.GetBaseItem().SpriteId == spriteId && Item.GetX == X && Item.GetY == Y)
