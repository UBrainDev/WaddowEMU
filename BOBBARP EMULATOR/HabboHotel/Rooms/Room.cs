...
namespace Plus.HabboHotel.Rooms
{
    public class Room : RoomData
    {
            ...
            foreach (Item item in this.GetRoomItemHandler().GetFurniObjects(Ball.GetX, Ball.GetY).ToList())
            {
                ...
                if (item.GetBaseItem().ItemName.StartsWith("fball_goal_g") && User != null && User.GetClient().GetHabbo().footballTeam != null)
                {
                    if(User.GetClient().GetHabbo().footballTeam == "green")
                    {
                        ...
                        {
                            Commentateur.Chat("Mais quelle erreur ! " + User.GetClient().GetHabbo().Username + " vient de marquer contre son camps !", false, 0);
                        }
                    ...
                    else if (User.GetClient().GetHabbo().footballTeam == "blue")
                    {
                        if (Commentateur != null)
                        {
                            Commentateur.Chat("Magnifique frappe de " + User.GetClient().GetHabbo().Username + " qui va directement dans les cages de l'équipe verte !", false, 0);
                        }
                        User.OnChat(User.LastBubble, "* Fait sa célébration *", true);
                        User.UnIdle();
                        User.DanceId = 0;
                ...
                else if (item.GetBaseItem().ItemName.StartsWith("fball_goal_b") && User != null && User.GetClient().GetHabbo().footballTeam != null)
                {
                    ...
                    else if (User.GetClient().GetHabbo().footballTeam == "green")
                    {
                        if (Commentateur != null)
                        {
                            Commentateur.Chat("Magnifique frappe de " + User.GetClient().GetHabbo().Username + " qui va directement dans les cages de l'équipe bleu !", false, 0);
                        }
                        User.OnChat(User.LastBubble, "* Fait sa célébration *", true);
                        User.UnIdle();
                        User.DanceId = 0;
        ...
        {
            string NameAppart = "Appartement de " + Session.GetHabbo().Username;
