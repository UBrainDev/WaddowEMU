...
{
    class RetrograderCommand : IChatCommand
    {
        ...
        {
            get { return "Rétrograder un civil"; }
        }
        ...
        public void Execute(GameClient Session, Room Room, string[] Params)
        {
            ...
            if (Params.Length < 2)
            {
                Session.SendWhisper("Syntaxe invalide, tapez :retrograder <pseudonyme>");
                return;
            }

            if (Session.GetHabbo().getCooldown("retrograder_command") == true)
            {
            ...
            if (NewRank == null)
            {
                Session.SendWhisper(TargetClient.GetHabbo().Username + " a déjà le rang le plus bas.");
                return;
            }
            ...
            Session.GetHabbo().addCooldown("retrograder_command", 2000);
            RoomUser User = Room.GetRoomUserManager().GetRoomUserByHabbo(Session.GetHabbo().Id);
            Group.retrograderRank(TargetClient.GetHabbo().Id);
            User.OnChat(User.LastBubble, "* Rétrograde " + TargetClient.GetHabbo().Username + " en tant que " + NewRank.Name + " dans l'entreprise " + Group.Name +  " *", true);
        }
    }
}