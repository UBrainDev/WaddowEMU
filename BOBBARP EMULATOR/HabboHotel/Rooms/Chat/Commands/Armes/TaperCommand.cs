...
{
    class TaperCommand : IChatCommand
    {
        ...
        public void Execute(GameClient Session, Room Room, string[] Params)
        {
            if (Params.Length == 1)
            {
                Session.SendWhisper("Syntaxe invalide, tapez :taper <pseudonyme>");
                return;
            }

            if (Session.GetHabbo().ArmeEquiped != "batte")
            {
                Session.SendWhisper("Vous devez vous équiper d'une batte pour pouvoir frapper un utilisateur avec.");
                return;
            }
            ...
            {
                Session.SendWhisper("Vous ne pouvez pas taper en dehors du ghetto.");
                return;
            }
            ...
            {
                Session.SendWhisper("Vous ne pouvez pas vous frappez vous même.");
                return;
            }
            ...
            {
                Session.SendWhisper("Vous ne pouvez pas taper lorsque vous êtes immobilisé.");
                return;
            }
            ...
            {
                Session.SendWhisper("Vous ne pouvez pas frapper un civil lorsque vous êtes immunisé.");
                return;
            }

            if (TargetUser.Immunised == true)
            {
                Session.SendWhisper("Vous ne pouvez pas frapper " + TargetClient.GetHabbo().Username + " car il est immunisé.");
                return;
            }

            if (User.DuelUser != null && TargetClient.GetHabbo().Username != User.DuelUser)
            {
                Session.SendWhisper("Vous ne pouvez pas frapper " + TargetClient.GetHabbo().Username + " car vous êtes en duel contre " + User.DuelUser + ".");
                return;
            }

            if (TargetUser.DuelUser != null && TargetUser.DuelUser != Session.GetHabbo().Username)
            {
                Session.SendWhisper("Vous ne pouvez pas frapper " + TargetClient.GetHabbo().Username + " car il est en duel.");
                return;
            }
            ...
            {
                User.OnChat(User.LastBubble, "* Tente de frapper " + TargetClient.GetHabbo().Username + " avec " + Name + " mais ne le touche pas [-2% ÉNERGIE] *", true);
            ...
            {
                User.OnChat(User.LastBubble, "* Frappe " + TargetClient.GetHabbo().Username + " avec " + Name + " lui causant " + degatTakedNumber + " points de dégats [-2% ÉNERGIE] *", true);
            ...
            else
            {
                ...
                User.OnChat(User.LastBubble, "* Frappe " + TargetClient.GetHabbo().Username + " avec " + Name + " et le tue [-2% ÉNERGIE] *", true);
